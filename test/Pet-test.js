...
describe('<Pet />', () => {
  describe('Rendering props', () => {
    ...
    it('should render the correct gender icon for male pets', () => {
      const wrapper = shallow(<Pet pet={MALE_DOG} />);
      expect(wrapper.text().includes(GENDER_ICON_MALE) && !wrapper.text().includes(GENDER_ICON_FEMALE)).to.be.true;
    });
  ...
  describe('Adopting a pet', () => {
    ...
    describe('Pet is already adopted', () => {
      it('should only show the already adopted button', () => {
        ...
        expect(wrapper.find('button.ui.disabled.button').length === 1 && wrapper.find('button.ui.primary.button').length === 0).to.be.true;
      });
